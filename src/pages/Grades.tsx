...
export default function Grades() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      {/* Tabla/Cards de calificaciones */}
      {grades.length > 0 ? (
        <div data-cy="grades-table">
          {/* Desktop table */}
          <div className="hidden sm:block bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-100">
                {grades.map(grade => {
                  const normalized = (grade.score / grade.maxScore) * 10;
                  return (
                    <tr key={grade.id} data-cy="grade-row">
                      <td className="p-4 text-sm">{grade.subject}</td>
                      <td className="p-4 text-sm">{grade.examName}</td>
                      <td className="p-4">
                        <span className={`font-medium text-sm ${normalized >= 7 ? 'text-green-600' : normalized >= 5 ? 'text-yellow-600' : 'text-red-600'}`}>
                          {grade.score}/{grade.maxScore}
                        </span>
                      </td>
                      <td className="p-4 text-sm">{grade.weight}%</td>
                      <td className="p-4 text-sm text-gray-500">{grade.date}</td>
                      <td className="p-4">
                        <button
          ...
          <div className="sm:hidden space-y-3">
            {grades.map(grade => {
              const normalized = (grade.score / grade.maxScore) * 10;
              return (
                <div key={grade.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4" data-cy="grade-row">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-900 text-sm truncate">{grade.subject}</p>
                      <p className="text-xs text-gray-500 mt-0.5">{grade.examName}</p>
                    </div>
                    <button
                      onClick={() => handleDeleteGrade(grade.id)}
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`text-lg font-bold ${normalized >= 7 ? 'text-green-600' : normalized >= 5 ? 'text-yellow-600' : 'text-red-600'}`}>
                      {grade.score}/{grade.maxScore}
                    </span>
                    <span className="text-xs text-gray-400">Peso: {grade.weight}%</span>
                    <span className="text-xs text-gray-400 ml-auto">{grade.date}</span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
                    <div
                      className={`h-1.5 rounded-full ${normalized >= 7 ? 'bg-green-500' : normalized >= 5 ? 'bg-yellow-500' : 'bg-red-500'}`}
                      style={{ width: `${Math.min(100, normalized * 10)}%` }}
                    />
                  </div>
                </div>
              );
        ...
        <div className="text-center py-12 bg-white rounded-xl border border-gray-100">
          ...
          <p className="text-gray-500 text-sm px-4">Registra tus notas para llevar un control de tu rendimiento académico</p>
        </div>
      )}
